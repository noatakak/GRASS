...
class SkillManager:
    def __init__(
        ...
        for i in self.control_primitives:
            regex_pattern = r'function\s+(\w+)\s*\('
            match = re.search(regex_pattern, i)
            if match:
                function_name = match.group(1)
                methName = function_name
            else:
                methName = "MineFlayer"

            description1 = self.generate_skill_description(methName, i)
            self.vectordb.add_texts(
                texts=[description1],
                ids=[methName],
                metadatas=[{"name": methName}],
            )
            self.skills[methName] = {
                "code": i,
